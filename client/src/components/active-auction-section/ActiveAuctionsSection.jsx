...
const ActiveAuctionsSection = () => {
  ...
  useEffect(() => {
    const fetchActiveAuctions = async () => {
      try {
        const response = await fetch(`${BASE_URL}/auction/active`);
        if (!response.ok) {
      ...
      <section className="active-auctions-section">
        <div className="active-auctions-container">
          <div className="active-auctions-header">
            <h2 className="active-auctions-title">Active Auctions</h2>
            <Link to="/catalog?status=active" className="see-all-link">
              Go to catalog →
            </Link>
          </div>
                  ...
                  <div className="auction-content">
                    ...
                    <div className="auction-footer">
                      <span className="auction-end-time">
                        Finish: {new Date(auction.endTime).toLocaleDateString()}
                      </span>
                      <Link
      ...
      <section className="upcoming-auctions-section">
        <div className="upcoming-auctions-container">
          <div className="upcoming-auctions-header">
            <h2 className="upcoming-auctions-title">Upcoming auctions</h2>
          </div>
        </div>
      </section>
    </>
