...
const createBid = async (req, res) => {
    try {
        ...
        if (!auction) {
            return res
                .status(400)
                .json({ success: false, message: "La subasta no existe" });
        }

        if (auction.status === "inactive") {
            return res.status(400).json({
                success: false,
                message: "La subasta aún no ha comenzado",
            });
        }
        if (auction.status === "closed") {
            return res
                .status(400)
                .json({ success: false, message: "La subasta ha finalizado" });
        }
        ...
        if (amount < minAllowed) {
            return res.status(400).json({
                success: false,
                message: `El monto mínimo permitido es ${minAllowed}`,
            });
        }
